...
void access_data(mem_addr_t addr) {      
    ...
    int set = (addr>>b) & Smask;
    int tag = addr>>(s+b);

    //Hits
    for(int l=0;l<E;l++){
	//valid and tag
	if(cache[set][l].valid && cache[set][l].tag==tag){
	    cache[set][l].lru_cnt=lru_cnt++;
	    hit_cnt++;
	    return;
	}
    }
    ...
    int index=0;
    for(int l=0;l<E;l++){
	if(cache[set][l].lru_cnt < cache[set][index].lru_cnt){
	    index=l;
	}
    }
    cache[set][index].tag=tag;
    cache[set][index].lru_cnt=lru_cnt;
}      
